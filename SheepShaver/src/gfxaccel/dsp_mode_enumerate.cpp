 ...
 *  backBufferDepthMask overlap; Tier 1 picks bit-depth (exact → deeper ≥
 *  requested → deepest available); Tier 2 picks resolution (exact →
 ...
 *
 *  Returns:
 *    - nullptr when s_dsp_modes is empty, when no mode's backBufferBestDepth
 *      overlaps req->backBufferDepthMask, or when depth_filtered would be
...
{
	/* --- Tier 0: backBufferDepthMask overlap ---------------------------
	 * Every candidate must have a depth bit present in
	 * req->backBufferDepthMask. If no mode satisfies the mask, bail out
	...
	for (const auto &m : s_dsp_modes) {
		uint32_t mode_depth_mask = DepthMaskForDepth(m.backBufferBestDepth);
		if ((req->backBufferDepthMask & mode_depth_mask) != 0) {
			pool.push_back(&m);
		}
	}
	if (pool.empty()) {
		DSP_LOG("FindBest: Tier 0 empty — req depthMask=0x%08x yields "
	 ...
	 * Primary: exact backBufferBestDepth match.
	...
	uint32_t exact_depth = req->backBufferBestDepth;
	for (auto *c : pool) {
		if (c->backBufferBestDepth == exact_depth) {
	...
	if (depth_filtered.empty()) {
		...
		for (auto *c : pool) {
			if (c->backBufferBestDepth >= exact_depth &&
			    c->backBufferBestDepth > best_depth) {
				best_depth = c->backBufferBestDepth;
			}
		}
		if (best_depth == 0) {
			/* Requested depth exceeds every mode — use deepest available. */
			for (auto *c : pool) {
				if (c->backBufferBestDepth > best_depth) {
					best_depth = c->backBufferBestDepth;
				}
			}
		}
