...
static inline int GLPixelSourceBytesPerPixel(uint32_t format, uint32_t type)
{
	if (type == GLPixelTypeUnsignedShort &&
	    (format == GLPixelFormatRGB || format == GLPixelFormatRGBA)) {
		return 2;
	}
...
static inline bool GLPixelLegacyUnsignedShortShouldUseIndexGrayFallback(
	int sampled_words,
	int duplicated_byte_words,
	int non_extreme_duplicated_byte_words,
	bool has_palette)
...
static inline bool GLPixelLegacyUnsignedShortShouldUse332Fallback(
	int sampled_words,
	int duplicated_byte_words,
	int non_extreme_duplicated_byte_words,
	bool has_palette)
{
	return GLPixelLegacyUnsignedShortShouldUseIndexGrayFallback(
		sampled_words, duplicated_byte_words,
		non_extreme_duplicated_byte_words, has_palette);
}
